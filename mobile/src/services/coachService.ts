...
export const coachService = {
  ...
  assignClient: async (clientId: number) => {
    const response = await api.post('/coach/assign-client', { client_id: clientId });
  ...
  createMesocycle: async (programId: number, data: Partial<Mesocycle>) => {
    const response = await api.post(`/coach/programs/${programId}/mesocycles`, data);
    return response.data as Mesocycle;
  },

  createMicrocycle: async (mesocycleId: number, data: Partial<Microcycle>) => {
    const response = await api.post(`/coach/mesocycles/${mesocycleId}/microcycles`, data);
    return response.data as Microcycle;
  },

  createSession: async (microcycleId: number, data: { name: string; day_of_week?: string; exercises: any[] }) => {
    const response = await api.post(`/coach/microcycles/${microcycleId}/sessions`, data);
